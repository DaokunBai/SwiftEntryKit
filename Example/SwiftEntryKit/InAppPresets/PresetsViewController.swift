...
class PresetsViewController: UIViewController {
    ...
    private func showFormView(attributes: EKAttributes) {
        ...
        let placeholderStyle = EKProperty.LabelStyle(font: MainFont.light.with(size: 14), color: UIColor(white: 0.5, alpha: 1))
        let textStyle = EKProperty.LabelStyle(font: MainFont.light.with(size: 14), color: .black)
        ...
        let button = EKProperty.ButtonContent(label: .init(text: "Continue", style: .init(font: MainFont.bold.with(size: 16), color: .white)), backgroundColor: .redish, highlightedBackgroundColor:  UIColor.redish.withAlphaComponent(0.8)) {
            SwiftEntryKit.dismiss()
        }
